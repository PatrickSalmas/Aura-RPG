                                       ...
                                       InTagsToDefs) const
{
	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		if (TypeDamage > -.5f) // .5 padding for floating point imprecision
		{
			...
			if (GameplayTags.DamageTypesToResistances.Contains(DamageType))
			{
                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	TagsToCaptureDefs.Add(Tags.Attributes_Secondary_ArmorPenetration, DamageStatics().ArmorDef);
	...
	for (const auto& Pair : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		if (UAuraAbilitySystemLibrary::IsRadialDamage(EffectContextHandle))
		{
			...
			// 2. Create delegate OnDamageDelegate, broadcast damage in TakeDamage  *
			// 3. Bind lambda to OnDamageDelegate on the Victim here   *
			// 4. Call UGameplayStatics::ApplyRadialDamageWithFalloff to cause damage (this will result in TakeDamage being call
			...
			UGameplayStatics::ApplyRadialDamageWithFalloff(TargetAvatar, DamageTypeValue, 0,
			                                               UAuraAbilitySystemLibrary::GetRadialDamageOrigin(
				                                               EffectContextHandle),
			                                               UAuraAbilitySystemLibrary::GetRadialDamageInnerRadius(
				                                               EffectContextHandle),
			                                               UAuraAbilitySystemLibrary::GetRadialDamageOuterRadius(
				                                               EffectContextHandle), 1.f, UDamageType::StaticClass(),
	...
	// TODO: Should check if CharacterClassInfo is not null. Or perhaps additionally, this code probably shouldn't run
	// at all if the Character is dead
